...
export default function SiteChrome({children,profile,socialLinks}:SiteChromeProps){
  ...
  return <div className="portfolio-app"><a className="skip-link" href="#main-content">Skip to content</a><div className="ambient-background"><div className="ambient-glow-1"/><div className="ambient-glow-2"/><div className="ambient-glow-3"/><div className="ambient-grid"/></div><Navbar onMenu={()=>setMenuOpen(true)} theme={theme} onToggleTheme={()=>setTheme(t=>t==='dark'?'light':'dark')}/><ResponsiveMenu open={menuOpen} onClose={()=>setMenuOpen(false)} theme={theme} onToggleTheme={()=>setTheme(t=>t==='dark'?'light':'dark')} socialLinks={socialLinks}/><main id="main-content">{children}</main><Footer profile={profile} socialLinks={socialLinks}/><Toast toasts={toasts} onDismiss={dismissToast}/></div>
}
