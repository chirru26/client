...
export default function Navbar({ onMenu, theme, onToggleTheme }: NavbarProps) {
  ...
  return <header className={`navbar-wrapper ${scrolled ? 'scrolled' : ''}`}><nav className={`navbar ${scrolled ? 'scrolled' : ''}`} aria-label="Main Navigation">
    ...
    <ul className="nav-links">{navLinks.map((link)=>{const active=pathname==='/'?activeSection===link.href.slice(1):pathname.startsWith(link.route);return <li key={link.label}><a href={`/${link.href}`} className={`nav-link ${active?'active':''}`} aria-current={active ? (pathname === '/' ? 'location' : 'page') : undefined} onClick={(e)=>handleNavLinkClick(e,link)}>{link.label}</a></li>})}</ul>
    <div className="nav-actions"><button type="button" className="theme-toggle-btn" onClick={onToggleTheme} aria-label="Toggle dark/light theme">{theme==='light'?<Moon size={16}/>:<Sun size={16}/>}</button><a href={portfolioApi.resumeDownloadUrl} target="_blank" rel="noreferrer" className="btn btn-primary btn-sm" onClick={()=>void portfolioApi.trackEvent('resume_download',{source:'navbar_cta'})}><FileText size={14}/> Resume</a><button type="button" className="mobile-menu-btn" onClick={onMenu} aria-label="Open mobile menu" aria-expanded={false} aria-controls="mobile-navigation"><Menu size={20}/></button></div>
  </nav></header>
}
