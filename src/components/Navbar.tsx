...
export default function Navbar({ onMenu, theme, onToggleTheme }: NavbarProps) {
  ...
  return <header className={`navbar-wrapper ${scrolled ? 'scrolled' : ''}`}><nav className={`navbar ${scrolled ? 'scrolled' : ''}`} aria-label="Main Navigation">
    ...
    <ul className="nav-links">{navLinks.map((link)=>{const active=pathname==='/'?activeSection===link.href.slice(1):pathname.startsWith(link.route);return <li key={link.label}><a href={`/${link.href}`} className={`nav-link ${active?'active':''}`} aria-current={active?'page':undefined} onClick={(e)=>handleNavLinkClick(e,link)}>{link.label}</a></li>})}</ul>
