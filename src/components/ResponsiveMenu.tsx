...
export default function ResponsiveMenu({open,onClose,theme,onToggleTheme,socialLinks=[]}:Props){
  ...
  return <AnimatePresence>{open&&<motion.div className="mobile-drawer-overlay" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose}><motion.aside id="mobile-navigation" className="mobile-drawer" initial={{x:'100%'}} animate={{x:0}} exit={{x:'100%'}} transition={{duration:.28,ease:'easeOut'}} onClick={(e)=>e.stopPropagation()}>
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}><button type="button" className="nav-brand" onClick={()=>{onClose();void(pathname==='/'?window.scrollTo({top:0,behavior:'smooth'}):router.push('/'))}} style={{background:'none',border:'none',cursor:'pointer',padding:0}}>chirru<span className="nav-brand-dot"/></button><button className="modal-close-btn" onClick={onClose} aria-label="Close menu"><X size={18}/></button></div>
    ...
    {socialLinks.length>0&&<div style={{display:'flex',flexWrap:'wrap',gap:8,padding:'10px 0'}}>{socialLinks.map(s=>{const plat=s.platform?.toLowerCase()||'';const isX=plat==='twitter'||plat==='x'||s.url.toLowerCase().includes('x.com')||s.url.toLowerCase().includes('twitter.com');return <a key={s.id||s.url} href={s.url} target="_blank" rel="noreferrer" className="social-icon-btn" style={{width:36,height:36}} aria-label={s.label||s.platform||'Social link'}>{plat==='github'&&<Github size={16}/>} {plat==='linkedin'&&<Linkedin size={16}/>} {isX&&<svg viewBox="0 0 24 24" width={14} height={14} fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>} {plat==='instagram'&&<Instagram size={16}/>} {plat==='email'&&<Mail size={16}/>} {!['github','linkedin','instagram','email'].includes(plat)&&!isX&&<Globe size={16}/>}</a>})}</div>}
